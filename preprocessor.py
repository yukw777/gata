...
class SpacyPreprocessor:
    ...
    def pad(self, unpadded_batch: List[List[int]]) -> List[List[int]]:
        max_len = max(len(word_ids) for word_ids in unpadded_batch)
        return [
            word_ids + [0] * (max_len - len(word_ids)) for word_ids in unpadded_batch
        ]

    def preprocess(self, batch: List[str]) -> List[List[int]]:
        return self.pad(
            [[self.word_to_id(token) for token in self.tokenize(s)] for s in batch]
        )
